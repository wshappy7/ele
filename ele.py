import requests
import json
import time
import pymysql
import re
connects=pymysql.connect(host='localhost',user='root',passwd='',port=3306,db='ele')
cur=connects.cursor()
headers = {
        'User-Agent': 'Mozilla/5.0 (Linux; Android 5.0; SM-G900P Build/LRX21T) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/75.0.3770.100 Mobile Safari/537.36',
        'Referer': 'https://h5.ele.me/',
        'X-Shard': 'loc=106.499063,29.586717, loc=106.499063,29.586717',
        'x-uab': '120#'
    }
#========导入cookie===============
from get_cookie import getCookie
from selenium import webdriver
while True:
    driver = webdriver.Chrome(executable_path=r'chromedriver.exe')
    try:
        c=getCookie(driver).get_phone()
        break
    except Exception as e:
        print(e)
        driver.close()

        pass
headers['Cookie']=c


#或取userid以便详情页提取
userid=re.findall(r'USERID=(.*?);',headers.get('Cookie'),re.S)[0]

#获取代理ip
from get_ip import CheckIp
proxies=CheckIp().check_ip()

#=====================导出城市经纬度====================
# import json
# with open('city.json',encoding='utf-8') as fp:  # with在不需要访问文件后将其关闭。
#     contents = json.loads(fp.read())
#     for con in contents:
#         name=con.get('name')
#         latitude=con.get('latitude')
#         longitude=con.get('longitude')
#         print(name,latitude,longitude)
#"name": "北京", "latitude": 39.90469, "longitude": 116.407173
#"name": "重庆", "latitude": 29.564711, "longitude": 106.550728
import redis
pool =redis.ConnectionPool(host='127.0.0.1',password='')
r=redis.Redis(connection_pool=pool)
while True:

    url=r.lpop('beijing').decode('utf8')#返回并删除列表的第一个value

    print(url)

    try:
        response=requests.get(url,headers=headers,verify=False,proxies=proxies,timeout=3).text
        time.sleep(2)
        print(response)
        if '用户网络信息异常' in response:
            for i in range(10):
                response = requests.get(url, headers=headers, verify=False, proxies=proxies, timeout=3).text
                if '用户网络信息异常' not in response:
                    break
                time.sleep(2)
        print(response)
        data=json.loads(response)
        item_list=data['items']
        for item in item_list:
            restaurant=item.get('restaurant')
            name=restaurant.get('name').strip()
            print(name)
            id = restaurant.get('id')
            print(id)
            url = f'https://h5.ele.me/pizza/shopping/restaurants/{id}/batch_shop?user_id={userid}&code=0.3191457519198255&extras=%5B%22activities%22%2C%22albums%22%2C%22license%22%2C%22identification%22%2C%22qualification%22%5D&terminal=h5&latitude=29.567921&longitude=106.496793'
            try:
                response = requests.get(url, headers=headers, verify=False,proxies=proxies,timeout=3).text
                print(response)

                if "请登录" in response:
                    driver.close()
                    while True:
                        driver = webdriver.Chrome(executable_path=r'chromedriver.exe')
                        try:

                            c = getCookie(driver).get_phone()
                            break
                        except Exception as e:
                            print(e)
                            driver.close()
                            pass
                    headers['Cookie'] = c
                    # 或取userid以便详情页提取
                    userid = re.findall(r'USERID=(.*?);', headers.get('Cookie'), re.S)[0]
                    url = f'https://h5.ele.me/pizza/shopping/restaurants/{id}/batch_shop?user_id={userid}&code=0.3191457519198255&extras=%5B%22activities%22%2C%22albums%22%2C%22license%22%2C%22identification%22%2C%22qualification%22%5D&terminal=h5&latitude=29.567921&longitude=106.496793'
                    response = requests.get(url, headers=headers, verify=False, proxies=proxies, timeout=3).text

                rst = json.loads(response).get('rst')
                rating = rst.get('rating')
                print(rating)
                recent_order_num = int(rst.get('recent_order_num'))
                print(recent_order_num)
                # sql = f"insert into test2(shopName,shopCode,shopScore,shopCount) values({repr(name)},{repr(id)},{rating},{recent_order_num})"
                sql = f"insert into test2(shopName,shopCode,shopScore,shopCount) values({repr(name)},{repr(id)},{rating},{recent_order_num})"
                cur.execute(sql)
                connects.commit()


            except Exception as e:
                print(e)
                print("详情页详情页详情页详情页详情页详情页详情页详情页")
                proxies = CheckIp().check_ip()






    except Exception as e:
        print(e)
        print("列表页列表页列表页列表页列表页列表页列表页列表页")

        # proxies = CheckIp().check_ip()






